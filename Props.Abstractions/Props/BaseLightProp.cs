		...
		where TModel : BaseLightPropVisualModel, IPropVisualModel, new()	
	{
		...
		protected async Task UpdateStringNodeCount(int nodeCount, ElementNode? propNode = null)
		{
			...
			if (existingNodes < nodeCount)
			{
				...
				//TODO PropertySetupHelper.AddOrUpdateColorHandling(newNodes, GetColorConfiguration());
			}
			else
			{
...

		{
			...
			//TODO PropertySetupHelper.AddOrUpdatePatchingOrder(propNode, startLocation, zigZag, zigZagOffset);
		...
		protected async Task AddOrUpdateColorHandling(IElementNode? node = null)
		{
			...
			try
			{
				...
				//TODO PropertySetupHelper.AddOrUpdateColorHandling(propNode, GetColorConfiguration());
			}
			catch (Exception e)
			{
				Console.WriteLine($"Error occured updating the color handling: {e.Message}");
				//TODO Logging.Error(e, "Error occured updating the color handling");
			}
