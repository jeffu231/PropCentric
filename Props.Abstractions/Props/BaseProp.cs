...
namespace Props.Abstractions.Props
{
		...
		where TModel : PropVisualModel, IPropVisualModel, new ()
	{
		...
		protected BaseProp(string name)
		{
			// Create the prop model
			PropVisualModel = new TModel();
			Id = Guid.NewGuid();
			PropComponents = new();
			_name = name;
		...
		/// </summary>
		/// <remarks>
		/// The <see cref="IPropVisualModel"/> represents the underlying data structure or configuration
		...
		/// </remarks>
		IPropVisualModel IProp.PropVisualModel => PropVisualModel;

		public TModel PropVisualModel
		{
			get;
